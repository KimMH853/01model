...
public class PurchaseDAO {
	 ...
	 * Connection con = DBUtil.getConnection();
	 * 
	 * String sql = "select * from USERS "; if (searchVO.getSearchCondition() !=
	 * null) { if (searchVO.getSearchCondition().equals("0")) { sql +=
	 * " where USER_ID='" + searchVO.getSearchKeyword() + "'"; } else if
	 * (searchVO.getSearchCondition().equals("1")) { sql += " where USER_NAME='" +
	 * searchVO.getSearchKeyword() + "'"; } } sql += " order by USER_ID";
	 * 
	 * PreparedStatement stmt = con.prepareStatement( sql,
	 ...
	 * 
	 * ArrayList<PurchaseVO> list = new ArrayList<PurchaseVO>(); if (total > 0) {
	 * for (int i = 0; i < searchVO.getPageUnit(); i++) { PurchaseVO vo = new
	 * PurchaseVO(); vo.setPurchaseId(rs.getString("USER_ID"));
	 * vo.setPurchaseName(rs.getString("USER_NAME"));
	 * vo.setPassword(rs.getString("PASSWORD")); vo.setRole(rs.getString("ROLE"));
	 * vo.setSsn(rs.getString("SSN")); vo.setPhone(rs.getString("CELL_PHONE"));
	 * vo.setAddr(rs.getString("ADDR")); vo.setEmail(rs.getString("EMAIL"));
	 ...
	 * 
	 * String sql =
	 * "update USERS set USER_NAME=?,CELL_PHONE=?,ADDR=?,EMAIL=? where USER_ID=?";
	 * 
	 * PreparedStatement stmt = con.prepareStatement(sql); stmt.setString(1,
	 * purchaseVO.getPurchaseName()); stmt.setString(2, purchaseVO.getPhone());
	 * stmt.setString(3, purchaseVO.getAddr()); stmt.setString(4,
	 * purchaseVO.getEmail()); stmt.setString(5, purchaseVO.getPurchaseId());
	 * stmt.executeUpdate();
	 * 
	 * con.close(); }
	 */
